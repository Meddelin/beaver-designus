...
function RenderNode({
  ...
  // Per-node error boundary: a DS component that throws (bad/missing prop
  // value, internal DS bug) degrades to an inline error chip for THAT node
  // only — the rest of the canvas keeps rendering. resetKey is the node's
  // props so the boundary retries when the agent corrects them.
  const rendered = (
