...
- **A5** — Internal-only packages to exclude. List every directory **directly under `<componentRoot>/`** that is **not** a UI component (analytics, hooks, core, internal-*, etc.). These become `excludePackages` in `manifest.config.json`. **Scope matters:** `excludePackages` only filters basenames *directly inside* `componentRoot`. A legacy/deprecated tree that lives **outside** `componentRoot` (e.g. react-ui-kit's root-level `deprecated/` or `tinkoff-packages/`) is **not** scanned anyway and listing it in `excludePackages` is a no-op — report its path but do NOT add it to `excludePackages` expecting an effect. Such trees, if a kept modern component imports them transitively, are handled at the Vite layer (legacy `.js`+JSX is pre-compiled; unparseable files degrade to a visible stub) — that is automatic, not your config task.
...
- **G2** — React provider requirements. If the DS needs a root `<ThemeProvider>` / `<DesignSystemProvider>` to render correctly, note the exact provider + package + required props. This is a **maintainer code change** (the preview render root) — the bring-up agent must **NOT** add it (zero-code rule). Flag it in the report under "Required next steps" so the maintainer ships provider support; until then those components may render unstyled/incorrectly (degraded, not crashing).
